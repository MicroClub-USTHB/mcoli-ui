...
Ensure your `@/lib/utils` or equivalent classname helper is correctly imported, and that `@/components/ui/mc-button` points to your `mc-button` (or compatible Button) component.
...
`McAlertDialogCancel` defaults to `variant="secondary"` and `size="md"`, but both can be overridden.

```tsx
<McAlertDialogCancel variant="secondary" size="sm">
  Go back
</McAlertDialogCancel>
```
